...
2026-06-24 [code] Bumped CI actions to clear the Node 20 deprecation warning: actions/checkout@v4->v7, actions/setup-python@v5->v6 (both run on node24, confirmed via action.yml). Drop-in for our basic usage; YAML valid.

2026-06-26 [note] Re-ran /code-improve whole-repo sweep (report-only): 5 reviewers + 2 simplifiers + 4 detect-only audits. 14 findings (0 high/6 med/8 low, no critical/security), 7 behavior-preserving simplifications. Audits clean (no artifacts/secrets, CI comprehensive). Report in CODE-IMPROVE-REPORT.md (untracked). Nothing changed.

2026-06-26 [code] Applied all 6 code-improve "suggested order of work" items: dsl _atom keeps only finite floats (nan/inf stay symbols); add_nogood guards empty literal list (no empty-clause unsat); de-duplicated assumptions_of_node/clause into shared core helpers used by LTMS+WatchedLTMS; tidied exercise dead code (ch10 ok, ch08 fired_a) + honest ch07 minimal-support docstring; cheap simplifications (dsl join/strip, jtms ternary, normalize and/or collapse). jtms retract contradiction-check: verified BPS jtms.lisp deliberately omits it -> no behavior change, documented parity. ruff+mypy clean, 161 tests pass. Uncommitted.

2026-06-26 [code] Code-improve batch 2 (rest of the report, branch code-improve-fixes): watched _retract re-installs in deterministic clause-index order; de-duplicated TRE/JTRE/LTRE get_dbclass into shared terms.index_symbol; normalize raises clear ValueError on malformed not/implies/iff arity and on empty taxonomy (was IndexError / silent unsat); dsl expect requires <expr> <status> + documented that proposition words cannot contain operator chars; watched is_true/is_false delegate to TmsNode; shared core.opposite_sign replaces duplicated _opposite in watched+cltms. +3 regression tests. ruff+mypy clean, 169 tests pass.

2026-06-28 [code] Applied code-improve fixes (uncommitted): terms._atom now keeps only finite floats (nan/inf stay symbols, mirrors dsl; the sibling reader missed last pass), dsl reuses terms._atom. Lockstep dedup: shared core.simplify_clause + clause_consequent used by LTMS+WatchedLTMS (dropped _TAUTOLOGY sentinel); added watched._two_watches, core._sat_pv_clauses, inlined add_nogood literals; dsl tokenizer uses next(), normalize._normalize_conjunction comprehension, ltre is_true/is_false/is_known share _signed_node. Asymmetry fixes: JTRE.assume raises on informant mismatch (matches LTRE), LTRE compound re-assume installs guard clause once (no per-cycle leak), LTRE.fetch preserves negated-pattern sign. Skipped TRE/JTRE/LTRE fetch/run_rules cross-module extraction and dds/indirect handler merge (intentional pedagogical parallels); jtms premise-overwrite is per the documented "premise wins" policy, not a bug. ruff+mypy clean, 169 tests pass (0 skipped, SAT-gated diff tests ran).
